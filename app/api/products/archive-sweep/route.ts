...
export async function POST(request: NextRequest) {
  ...
  const [{ data: products }, { data: recentSales }] = await Promise.all([
    db.from('products').select('id, archived, stock_quantity, stock_full, stock_fba, stock_shopee').limit(3000),
  ...
  for (const p of products ?? []) {
    const temEstoque = Number(p.stock_quantity ?? 0) + Number(p.stock_full ?? 0) + Number((p as any).stock_fba ?? 0) + Number((p as any).stock_shopee ?? 0) > 0
