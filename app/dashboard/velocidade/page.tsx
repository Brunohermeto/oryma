...
export default async function VelocidadePage() {
  ...
  const { data: products } = await db.from('products').select('id, name, sku, stock_quantity, stock_full, stock_fba, stock_shopee, archived, updated_at').eq('archived', false)
  ...
  const productRows = (products ?? []).map(product => {
    ...
    const fullStock  = Number((product as any).stock_full ?? 0) + Number((product as any).stock_fba ?? 0) + Number((product as any).stock_shopee ?? 0)
