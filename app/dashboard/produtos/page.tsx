...
export default async function ProdutosPage() {
  ...
  const [{ data: products }, { data: allCmps }, yearSales] = await Promise.all([
    db.from('products').select('id, name, sku, stock_quantity, stock_full, stock_fba, archived').order('name'),
    db.from('cmp_costs')
  ...
  const rows: ProductRow[] = (products ?? []).map(p => {
    ...
    return {
      ...
      stockFull: Number((p as any).stock_full ?? 0) + Number((p as any).stock_fba ?? 0),
      sold12m: sold,
